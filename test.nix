...
pkgs.testers.nixosTest {
  ...
  nodes.machine =
    ...
    }:
    {
      ...
      # Serve the URL from a local file we can control
      services.nginx = {
        enable = true;
        virtualHosts.default.root = "/";
      };
  ...
  testScript =
    ...
    ''
      ...
      machine.succeed("journalctl -I -u nginx.service")
      ...
      with subtest("Missing state and same upstream"):
        ...
        machine.succeed("echo '${base-system}' > /upstream")
      ...
      with subtest("Missing state and differing upstream"):
        ...
        machine.succeed("rm -f /run/deployed-system")
        machine.succeed("echo '${next-system}' > /upstream")
